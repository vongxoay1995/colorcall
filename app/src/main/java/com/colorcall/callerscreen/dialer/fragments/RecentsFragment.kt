...
import com.colorcall.callerscreen.dialer.MIN_RECENTS_THRESHOLD
    ...
    private fun gotRecents(recents: List<RecentCall>) {
        if (recents.isEmpty()) {
            ...
            if (binding.recentsList.adapter == null) {
                recentsAdapter = RecentCallsAdapter(activity as DialerActivity, recents.toMutableList(), binding.recentsList, this, true) {
                    val recentCall = it as RecentCall
                    if (context.config.showCallConfirmation) {
                        CallConfirmationDialog(activity as DialerActivity, recentCall.name) {
