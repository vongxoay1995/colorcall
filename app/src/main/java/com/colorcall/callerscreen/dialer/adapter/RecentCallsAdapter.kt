package com.colorcall.callerscreen.dialer.adapter
...
import com.colorcall.callerscreen.databinding.ItemRecentCallBinding
...
import com.colorcall.callerscreen.dialer.dialog.ShowGroupedCallsDialog
...
class RecentCallsAdapter(
    ...
    private fun callContact() {
        val phoneNumber = getSelectedPhoneNumber() ?: return
        (activity as DialerActivity).startCallIntent(phoneNumber)
    }
    ...
    private fun findContactByCall(recentCall: RecentCall): Contact? {
        return (activity as CallOwnerActivity).cachedContacts.find { it.name == recentCall.name && it.doesHavePhoneNumber(recentCall.phoneNumber) }
    }
